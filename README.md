...
[![Version](https://img.shields.io/badge/version-1.0.0-blue.svg)](https://github.com/linghuchong6668/Code-Churn-Insights)
[![License](https://img.shields.io/badge/license-MIT-green.svg)](https://opensource.org/licenses/MIT)
[![VS Code](https://img.shields.io/badge/VS%20Code-%3E%3D1.75.0-007ACC.svg)](https://code.visualstudio.com/)
[![Gitee](https://img.shields.io/badge/Gitee-Mirror-red.svg)](https://gitee.com/linghuchong6668/vscode_extension)
...
2. **打开项目**：在 VS Code 中打开任意 Git 仓库
3. **运行分析**：按 `Cmd+Shift+P` (Mac) 或 `Ctrl+Shift+P` (Windows)，输入 `Code Churn: 分析当前仓库`
4. **查看报告**：自动生成可视化报告，点击文件即可跳转查看代码

就这么简单！🎉 无需任何配置，立即看到项目中的高风险代码文件。

### 📊 四象限风险模型

插件使用独创的**四象限风险模型**，将文件按照"变动频率"和"Bug 修复频率"分类：

- 🔴 **危险区域**（高变动 + 高Bug）：这些文件经常改动且频繁出错，**需要立即重构**
- 🟢 **活跃区域**（高变动 + 低Bug）：健康的开发区域，虽然经常改动但质量稳定
- 🟡 **不稳定区域**（低变动 + 高Bug）：不常改动但一改就出问题，**技术债务的标志**
- 🔵 **稳定区域**（低变动 + 低Bug）：项目的核心稳定代码，可放心参考

### 🎯 解决什么问题？

- **代码审查没重点**？→ 优先审查红色和黄色区域的文件
- **不知道从哪重构**？→ 按风险等级排序，优先重构危险文件
...
- **零学习成本**：一键分析，3 秒看到结果，无需任何配置
- **100% 基于 Git**：利用现有提交历史，不需要额外工具或服务
- **数据驱动决策**：用客观数据替代主观判断，让重构有的放矢
- **深度集成 IDE**：在 VS Code 中无缝使用，点击图表直接跳转代码
...
```bash
# 克隆仓库
git clone https://github.com/linghuchong6668/Code-Churn-Insights.git
...
- **GitHub Issues**：https://github.com/linghuchong6668/Code-Churn-Insights/issues
- **Gitee Issues**：https://gitee.com/linghuchong6668/vscode_extension/issues
